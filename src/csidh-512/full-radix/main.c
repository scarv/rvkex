...
void test_fp()
{
  ...
  printf("- uint sqr ps:");
  LOAD_CACHE(uint_sqr2_ps_sw(t, &a), 1000);
  MEASURE_CYCLES(uint_sqr2_ps_sw(t, &a), 10000);
  printf("       #cycle = %lld\n", diff_cycles);
  ...
  printf("- uint sqr ps:");
  LOAD_CACHE(uint_sqr2_ps_ise(t, &a), 1000);
  MEASURE_CYCLES(uint_sqr2_ps_ise(t, &a), 10000);
  printf("       #cycle = %lld\n", diff_cycles);
...
void test_action()
{
  ...
  public_key pub_alice[bench_num];
  ...
  printf("- csidh:");
  MEASURE_CYCLES(ret = csidh(&pub_alice[i], &base, &priv_alice[i]), bench_num);
  printf("             #cycle = %lld\n", diff_cycles);
