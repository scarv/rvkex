...
void xTPL_v0(const point_proj_t P, point_proj_t Q, const f2elm_t A24minus, const f2elm_t A24plus)
{
  f2elm_t t0, t1, t2, t3, t4, t5, t6;
                                        //  range | limb-length | computation 
  mp2_sub_p2(t0, P->X, P->Z);           // 2p->4p |  56    ->56 | t0 = X-Z
  ...
  mp2_sub_p2(t1, t1, t3);               // 2p->4p |  56    ->56 | t1 = 4*X^2 - (X+Z)^2
  mp2_sub_p2(t1, t1, t2);               // 4p->6p |  56    ->56 | t1 = 4*X^2 - (X+Z)^2 - (X-Z)^2
  ...
  mp2_sub_p2(t3, t2, t3);               // 2p->4p |  56    ->56 | t3 = A24minus*(X-Z)^4 - A24plus*(X+Z)^4
  mp2_sub_p2(t2, t5, t6);               // 2p->4p |  56    ->56 | t2 = A24plus*(X+Z)^2 - A24minus*(X-Z)^2
  fp2mul_mont_v0(t1, t1, t2);           // 6p->2p |  56->56->56 | t1 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]
  fp2add(t2, t3, t1);                   // 4p->2p |  56    ->56 | t2 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2] + A24minus*(X-Z)^4 - A24plus*(X+Z)^4
  fp2sqr_mont_v0(t2, t2);               // 2p->2p |  56->56->56 | t2 = t2^2
  fp2mul_mont_v0(Q->X, t4, t2);         // 4p->2p |  56->56->56 | X3 = 2*X*t2
  fp2sub(t1, t3, t1);                   // 4p->2p |  56    ->56 | t1 = A24minus*(X-Z)^4 - A24plus*(X+Z)^4 - [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]
...
{
                                            //  range | limb-length | computation 
  mp2_sub_p2(coeff[1], P->X, P->Z);         // 2p->4p |  56    ->56 | coeff[1] = X4-Z4
...
void eval_4_isog_v0(point_proj_t P, f2elm_t *coeff)
{
  ...
  mp2_sub_p2(t1, P->X, P->Z);           // 2p->4p |  56    ->56 | t1 = X-Z
  ...
  mp2_sub_p2(P->Z, P->X, P->Z);         // 2p->4p |  56    ->56 | Z  = (X-Z)*coeff[2] - (X+Z)*coeff[1]
  ...
  mp2_sub_p2(t0, P->Z, t0);             // 2p->4p |  56    ->56 | t0 = [(X-Z)*coeff[2] - (X+Z)*coeff[1]]^2 - coeff[0]*(X+Z)*(X-Z)
...
void get_3_isog_v0(const point_proj_t P, f2elm_t A24minus, f2elm_t A24plus, f2elm_t* coeff)
{
  f2elm_t t0, t1, t2, t3, t4;
                                        //  range | limb-length | computation 
  mp2_sub_p2(coeff[0], P->X, P->Z);     // 2p->4p |  56    ->56 | coeff0 = X-Z
...
void eval_3_isog_v0(point_proj_t Q, const f2elm_t *coeff)
{
  ...
  mp2_sub_p2(t1, Q->X, Q->Z);           // 2p->4p |  56    ->56 | t1 = X-Z
  ...
  mp2_sub_p2(t0, t1, t0);               // 2p->4p |  56    ->56 | t0 = coeff1*(X-Z) - coeff0*(X+Z)
...
void xDBLADD_v0(point_proj_t P, point_proj_t Q, const f2elm_t XPQ, const f2elm_t ZPQ, const f2elm_t A24)
{
  ...
  mp2_sub_p2(t1, P->X, P->Z);           // 2p->4p |  56    ->56 | t1 = XP-ZP
  fp2sqr_mont_v0(P->X, t0);             // 4p->2p |  56->56->56 | XP = (XP+ZP)^2
  mp2_sub_p2(t2, Q->X, Q->Z);           // 2p->4p |  56    ->56 | t2 = XQ-ZQ
  ...
  mp2_sub_p2(t2, P->X, P->Z);           // 2p->4p |  56    ->56 | t2 = (XP+ZP)^2-(XP-ZP)^2
  ...
  mp2_sub_p2(Q->Z, t0, t1);             // 2p->4p |  56    ->56 | ZQ = (XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)
...
void get_2_isog_v0(const point_proj_t P, f2elm_t A, f2elm_t C)
{
                                        //  range | limb-length | computation 
  fp2sqr_mont_v0(A, P->X);              // 2p->2p |  56->56->56 | A = X2^2
  fp2sqr_mont_v0(C, P->Z);              // 2p->2p |  56->56->56 | C = Z2^2
  mp2_sub_p2(A, C, A);                  // 2p->4p |  56    ->56 | A = Z2^2 - X2^2
}

void get_2_isog_v1(const point_proj_t P, f2elm_t A, f2elm_t C)
{
                                        //  range | limb-length | computation 
  fp2sqr_mont_v1(A, P->X);              // 2p->2p |  56->57->56 | A = X2^2
  fp2sqr_mont_v1(C, P->Z);              // 2p->2p |  56->57->56 | C = Z2^2
  mp2_sub_p2(A, C, A);                  // 2p->4p |  56    ->56 | A = Z2^2 - X2^2
}

void eval_2_isog_v0(point_proj_t P, point_proj_t Q)
{
  f2elm_t t0, t1, t2, t3;
                                        //  range | limb-length | computation 
  mp2_add_v0(t0, Q->X, Q->Z);           // 2p->4p |  56    ->56 | t0 = X2+Z2
  mp2_sub_p2(t1, Q->X, Q->Z);           // 2p->4p |  56    ->56 | t1 = X2-Z2
  mp2_add_v0(t2, P->X, P->Z);           // 2p->4p |  56    ->56 | t2 = X+Z
  mp2_sub_p2(t3, P->X, P->Z);           // 2p->4p |  56    ->56 | t3 = X-Z 
  fp2mul_mont_v0(t0, t0, t3);           // 4p->2p |  56->56->56 | t0 = (X2+Z2)*(X-Z)
  fp2mul_mont_v0(t1, t1, t2);           // 4p->2p |  56->56->56 | t1 = (X2-Z2)*(X+Z)  
  mp2_add_v0(t2, t0, t1);               // 2p->4p |  56    ->56 | t2 = (X2+Z2)*(X-Z) + (X2-Z2)*(X+Z)
  mp2_sub_p2(t3, t0, t1);               // 2p->4p |  56    ->56 | t3 = (X2+Z2)*(X-Z) - (X2-Z2)*(X+Z)  
  fp2mul_mont_v0(P->X, P->X, t2);       // 4p->2p |  56->56->56 | Xfinal
  fp2mul_mont_v0(P->Z, P->Z, t3);       // 4p->2p |  56->56->56 | Zfinal
}

void eval_2_isog_v1(point_proj_t P, point_proj_t Q)
{
  f2elm_t t0, t1, t2, t3;
                                        //  range | limb-length | computation 
  mp2_add_v1(t0, Q->X, Q->Z);           // 2p->4p |  56    ->57 | t0 = X2+Z2
  mp2_sub_p2(t1, Q->X, Q->Z);           // 2p->4p |  56    ->56 | t1 = X2-Z2
  mp2_add_v1(t2, P->X, P->Z);           // 2p->4p |  56    ->57 | t2 = X+Z
  mp2_sub_p2(t3, P->X, P->Z);           // 2p->4p |  56    ->56 | t3 = X-Z 
  fp2mul_mont_v1(t0, t0, t3);           // 4p->2p |  56,57 ->56 | t0 = (X2+Z2)*(X-Z)
  fp2mul_mont_v1(t1, t1, t2);           // 4p->2p |  56,57 ->56 | t1 = (X2-Z2)*(X+Z)  
  mp2_add_v1(t2, t0, t1);               // 2p->4p |  56    ->57 | t2 = (X2+Z2)*(X-Z) + (X2-Z2)*(X+Z)
  mp2_sub_p2(t3, t0, t1);               // 2p->4p |  56    ->56 | t3 = (X2+Z2)*(X-Z) - (X2-Z2)*(X+Z)  
  fp2mul_mont_v1(P->X, P->X, t2);       // 4p->2p |  56,57 ->56 | Xfinal
  fp2mul_mont_v1(P->Z, P->Z, t3);       // 4p->2p |  56->57->56 | Zfinal
}
