...
{
  fpcopy(XP[0], gen           );
  fpcopy(XP[1], gen +   NLMB56);
  fpcopy(XQ[0], gen + 2*NLMB56);
  fpcopy(XQ[1], gen + 3*NLMB56);
  fpcopy(XR[0], gen + 4*NLMB56);
...
{
  point_proj_t R, phiP = { 0 }, phiQ = { 0 }, phiR = { 0 }, pts[MAX_INT_POINTS_ALICE], S;
  ...
  mp2_add_v1(A24plus, A24plus, A24plus);// A24plus = 2
  mp2_add_v0(C24, A24plus, A24plus);    // C24     = 4
  mp2_add_v0(A, A24plus, C24);          // A       = 6
  mp2_add_v0(A24plus, C24, C24);        // A24plus = 8
  ...
  fp2mul_mont_v1(phiP->X, phiP->X, phiP->Z);
  fp2mul_mont_v1(phiQ->X, phiQ->X, phiQ->Z);
  fp2mul_mont_v1(phiR->X, phiR->X, phiR->Z);
...
void EphemeralKeyGeneration_B(const unsigned char* PrivateKeyB, unsigned char* PublicKeyB)
{
  ...
  fpcopy(A24plus[0], mont_R);             // A24plus  = 1
  mp2_add_v1(A24plus, A24plus, A24plus);  // A24plus  = 2
  mp2_add_v0(A24minus, A24plus, A24plus); // A24minus = 4
  mp2_add_v0(A, A24plus, A24minus);       // A        = 6
  mp2_add_v0(A24plus, A24minus, A24minus);// A24plus  = 8
...
{
  point_proj_t R, pts[MAX_INT_POINTS_ALICE], S;
  ...
  mp_add_v1(C24[0], mont_R, mont_R);    // C24 = 2C
  mp2_add_v0(A24plus, A, C24);          // A24plus = A+2C
  mp_add_v0(C24[0], C24[0], C24[0]);    // C24 = 4C
  ...
  get_4_isog(R, A24plus, C24, coeff); 
  mp2_add_v0(A24plus, A24plus, A24plus);
